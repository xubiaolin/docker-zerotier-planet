...
docker buildx build --platform linux/arm64,linux/amd64 --build-arg TAG="${latest_tag}" -t "$DOCKER_IMAGE":latest --push .
docker buildx build --platform linux/arm64,linux/amd64 --build-arg TAG="${latest_tag}" -t "${DOCKER_IMAGE}:${latest_tag}" --push .
